...
client = OpenAI(
    base_url="https://openrouter.ai/api/v1",
    api_key="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"
)
...
class OpenRouterBlogAI(View):
    ...
    def post(self, request):
        try:
            ...
            system_prompt = f"""
You are a highly skilled, creative multilingual editor specializing in {language}. 
Your goal is to help local authors create professional, catchy, and deep blog headers and improve their content.

THE TASK:
1. Generate a creative, meaningful, and professional BLOG TITLE in {language}.
2. Provide a polished, grammatically correct version of the BLOG CONTENT.

CONTEXT:
- Current Title (if any): "{existing_title}"
- Content Language: {language}

CRITICAL LINGUISTIC RULES for {language}:
- DO NOT translate literally. 
- AVOID robotic or textbook-style phrasing. Use natural, modern, and engaging {language}.
- The title should be short (1–6 words), abstract, and thought-provoking.
- If an existing title is provided, try to IMPROVE it rather than ignoring it, unless it is completely irrelevant.
- DO NOT use English words unless they are standard in modern {language} blog terminology.
- Both the title and content MUST be strictly in {language}.

OUTPUT FORMAT:
- Reply ONLY with a valid JSON object.
- The JSON object must have exactly two keys: "title" and "corrected_text".
- DO NOT include any text, quotes, or explanations outside the JSON block.

Example JSON:
{{
  "title": "A beautiful, evocative heading here",
  "corrected_text": "Polished and fluent blog content here"
}}
"""
            ...
            import re
            title = ""
            corrected_text = blog_content # Default to original

            try:
                # 1. Try to find the largest JSON block
                ...
                if json_match:
                    ...
                    try:
                        ...
                        corrected_text = res_data.get("corrected_text", "").strip()
                    except json.JSONDecodeError:
                        # 2. If json.loads fails, try to extract values using targeted regex
                        title_match = re.search(r'"title"\s*:\s*"([^"]*)"', json_str)
                        content_match = re.search(r'"corrected_text"\s*:\s*"([^"]*)"', json_str)
                        
                        if title_match: title = title_match.group(1)
                        if content_match: corrected_text = content_match.group(1)
                
                # 3. Fallback if still empty
                if not title:
                    # Clean up common JSON junk from the raw text
                    clean_text = re.sub(r'["\'{}]', '', ai_response) # remove braces and quotes
                    clean_text = re.sub(r'title\s*:\s*', '', clean_text, flags=re.IGNORECASE)
                    clean_text = re.sub(r'corrected_text\s*:\s*', '', clean_text, flags=re.IGNORECASE)
                    
                    lines = [l.strip() for l in clean_text.split('\n') if l.strip()]
                    title = lines[0] if lines else "Untitled"
                    
            except Exception as e:
                print(f"AI Parse Error: {e}. Raw: {ai_response}")
                title = "Untitled"

            # Final cleanup of any trailing commas or braces that escaped
            title = title.strip(' ,;{}[]"')
            corrected_text = (corrected_text or blog_content).strip()
